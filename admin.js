...
if(!ready){message($("#loginMessage"),"Firebase 프로젝트 연결 정보가 아직 설정되지 않았습니다.","error");$("#signIn").disabled=true;}else{
  app=initializeApp(config);auth=getAuth(app);db=getFirestore(app);storage=getStorage(app);window.SAEROM_ADMIN_CONTEXT={auth,db,storage};window.dispatchEvent(new CustomEvent("saerom-admin-ready",{detail:window.SAEROM_ADMIN_CONTEXT}));
