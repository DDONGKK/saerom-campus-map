(function () {
  ...
  function search(q){const n=norm(q),classroom=classroomQuery(q);if(!n)return[];const out=[];D.places.filter(p=>p.active&&p.searchable).map(data=>{const exactFields=[data.name,data.koName,data.nameEn,data.room,data.category,data.categoryEn,data.description,data.descriptionEn,...(data.aliases||[])],nameFields=[data.name,data.koName,data.nameEn,...(data.aliases||[])];const score=classroom&&nameFields.some(x=>norm(x)===norm(classroom))?4:exactFields.some(x=>norm(x).includes(n))?3:nameFields.some(x=>sharesTwoChars(n,x))?1:0;return{type:"장소",data,score};}).filter(result=>result.score).sort((a,b)=>b.score-a.score||a.data.floor-b.data.floor||String(a.data.room).localeCompare(String(b.data.room),"ko",{numeric:true})).forEach(result=>out.push({type:result.type,data:result.data}));D.departments.filter(d=>[d.name,d.duties,...(d.aliases||[])].some(x=>norm(x).includes(n))).forEach(data=>out.push({type:"부서",data}));return out;}
