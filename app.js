(function () {
  ...
  function corridorDuration(floor,from,to){const points=corridorPoints(floor,from,to);let distance=0;for(let i=1;i<points.length;i++)distance+=Math.hypot(points[i][0]-points[i-1][0],points[i][1]-points[i-1][1]);return`${Math.max(3,distance/3).toFixed(2)}s`;}
