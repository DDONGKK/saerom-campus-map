(function () {
  ...
  const CONNECTORS={stairs:{1:[49,64],2:[61,76],3:[61,76],4:[50,31],5:[52,77]},elevator:{1:[47,38],2:[62,42],3:[62,46],4:[53,69],5:[45,43]}};
  const CORRIDOR_SPINE={1:61,2:68,3:64,4:55,5:64};
  function roomAccess(place){const spine=CORRIDOR_SPINE[place.floor]||50,x=Number(place.mapX),y=Number(place.mapY);if(Math.abs(x-spine)<9)return[x,y];return x<spine?[spine-5,y]:[spine+5,y];}
  function routeStartPoint(route){if(route.placeId){const p=placeById(route.placeId);if(p)return roomAccess(p);}if(route.id==="gate")return[50,92];if(route.id==="lobby")return[58,80];return(CONNECTORS.stairs[route.floor]||[50,80]).slice();}
  function corridorPath(floor,from,to){const spine=CORRIDOR_SPINE[floor]||50;return`M ${from[0]} ${from[1]} L ${spine} ${from[1]} L ${spine} ${to[1]} L ${to[0]} ${to[1]}`;}
