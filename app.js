(function () {
  ...
  function routeStartPoint(route){if(route.placeId){const p=placeById(route.placeId);if(p)return[p.mapX,p.mapY];}if(route.id==="gate")return[50,94];if(route.id==="lobby")return[50,88];return(CONNECTORS.stairs[route.floor]||[50,80]).slice();}
  function corridorPath(from,to){const midX=Math.max(20,Math.min(80,50));return`M ${from[0]} ${from[1]} L ${midX} ${from[1]} L ${midX} ${to[1]} L ${to[0]} ${to[1]}`;}
  function buildRouteStages(){const s=D.routes.find(x=>x.id===$("#routeStart").value),p=placeById($("#routeEnd").value);if(!s||!p)return[];const connectorType=routeGuide.injured?"elevator":"stairs",connectorStart=CONNECTORS[connectorType][s.floor],connectorEnd=CONNECTORS[connectorType][p.floor],start=routeStartPoint(s),end=[p.mapX,p.mapY];if(s.floor===p.floor)return[{floor:p.floor,from:start,to:end,final:true}];return[{floor:s.floor,from:start,to:connectorStart,final:false},{floor:p.floor,from:connectorEnd,to:end,final:true}];}
  ...
  function renderRouteStage(){const panel=$("#routeMapPanel"),stage=routeGuide.stages[routeGuide.index];if(!routeGuide.active||!stage){panel.hidden=true;return;}panel.hidden=false;const floor=D.floors.find(f=>f.number===stage.floor),destination=placeById($("#routeEnd").value),overlay=$("#routeOverlay"),pathData=corridorPath(stage.from,stage.to);$("#routeMapImage").src=floor.image;$("#routeMapTitle").textContent=text(`${stage.floor}층 경로`,`Floor ${stage.floor} route`);$("#routeModeBadge").textContent=routeGuide.injured?I18N[state.lang].safeRoute:I18N[state.lang].normalRoute;overlay.replaceChildren();const outline=svgNode("path",{d:pathData,class:"route-line-outline"}),corridor=svgNode("path",{d:pathData,class:"route-line-corridor"}),finish=svgNode("circle",{cx:stage.to[0],cy:stage.to[1],r:"1.8",class:"route-finish"}),character=svgNode("image",{href:routeGuide.injured?"assets/characters/route-injured.png":"assets/characters/route-walker.png",width:"11",height:"11",x:"-5.5",y:"-9",class:"route-character",preserveAspectRatio:"xMidYMid meet"}),motion=svgNode("animateMotion",{dur:routeGuide.injured?"8s":"6s",repeatCount:"indefinite",path:pathData,rotate:"auto"});character.append(motion);overlay.append(outline,corridor,finish,character);const more=!stage.final;$("#routeStageText").textContent=more?text(`${stage.floor}층의 표시된 복도를 따라 ${routeGuide.injured?"E.V 엘리베이터":"중앙 계단"} 앞까지 이동하세요. 도착한 뒤 아래 버튼을 눌러 주세요.`,`Follow the marked corridor to ${routeGuide.injured?"the E.V elevator":"the central stairs"}, then confirm below.`):text(`${stage.floor}층의 표시된 베이지색 복도를 따라 ${destination.room}호 ${placeName(destination)}까지 이동하세요.`,`Follow the marked beige corridor to Room ${destination.room}, ${placeName(destination)}.`);$("#routeStageDone").hidden=!more;panel.scrollIntoView({behavior:"smooth",block:"start"});}
