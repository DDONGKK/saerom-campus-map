(function () {
  ...
  function renderBillboard(){const board=$("#eventBillboard"),track=$("#eventBillboardTrack");if(!board||!track)return;const now=Date.now(),messages=(window.SAEROM_EVENTS||[]).filter(event=>event.active!==false&&(!event.endAt||new Date(event.endAt).getTime()>=now)).map(event=>state.lang==="en"?(event.billboardTextEn||event.titleEn||event.title):(event.billboardText||event.title)).filter(Boolean);board.hidden=!messages.length;track.replaceChildren();if(!messages.length)return;const message=`★ ${messages.join("     ◆     ")}     `;track.style.setProperty("--billboard-duration",`${Math.max(18,message.length*.34)}s`);track.append(el("span",{text:message}),el("span",{text:message,"aria-hidden":"true"}));}
  ...
  function search(q){const n=norm(q);if(!n)return[];const out=[];D.places.filter(p=>p.active&&p.searchable&&[p.name,p.nameEn,p.room,p.category,p.categoryEn,p.description,p.descriptionEn].some(x=>norm(x).includes(n))).forEach(data=>out.push({type:"장소",data}));D.departments.filter(d=>[d.name,d.duties].some(x=>norm(x).includes(n))).forEach(data=>out.push({type:"부서",data}));return out;}
  ...
  function roomAccess(place){const graph=corridorGraph(place.floor),key=closestCorridorNode(graph,[Number(place.mapX),Number(place.mapY)]);return key?graph.nodes.get(key):[Number(place.mapX),Number(place.mapY)];}
