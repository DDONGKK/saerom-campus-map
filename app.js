(function () {
  ...
  Object.assign(I18N.ko,{healthMode:"몸이 아파요",healthyMode:"아프지 않아요",visualRoute:"지도 길안내",stageDone:"이 층 이동 완료",normalRoute:"일반 경로 · 계단 이용",safeRoute:"안전 경로 · E.V 이용"});
  ...
  const CORRIDOR_SEGMENTS={
    ...
    2:[[54,24,70,24],[54,24,54,85],[70,24,70,90],[40,38,70,38],[48,49,73,49],[54,46,57,46],[54,46,54,49],[47,68,70,68],[54,84,70,84],[76,68,76,90]],
  ...
  function roomAccess(place){return[Number(place.mapX),Number(place.mapY)];}
  function routeStartPoint(route){if(route.placeId){const p=placeById(route.placeId);if(p)return roomAccess(p);}if(route.id==="gate")return[50,92];if(route.id==="lobby")return[50,80];return(CONNECTORS.stairs[route.floor]||[50,80]).slice();}
