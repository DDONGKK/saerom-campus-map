(function () {
  ...
  Object.assign(I18N.ko,{eventMenu:"행사 일정 안내",eventMenuDesc:"학교 행사와 장소 확인",reportMenu:"오류 제보",reportMenuDesc:"잘못된 정보를 알려주세요",eventTitle:"행사 일정 안내",eventEmpty:"현재 행사를 진행하고 있지 않습니다.",reportTitle:"오류 제보",reportType:"오류 종류",reportMap:"지도·길안내 오류",reportPlace:"장소 정보 오류",reportQuest:"탐방 미션 오류",reportOther:"기타 오류",reportContent:"오류 내용",reportPlaceholder:"어느 화면에서 어떤 오류가 있었는지 자세히 적어주세요.",reportContact:"연락처 또는 이메일 (선택)",reportContactPlaceholder:"답변이 필요한 경우에만 입력하세요.",reportSubmit:"완료",reportWarning:"⚠ 욕설 및, 장난 식 제보는 하지 말아주세요."});
  ...
  function setupErrorReport(){const form=$("#reportForm"),messageBox=$("#reportMessage");if(!form)return;form.onsubmit=async event=>{event.preventDefault();const content=$("#reportContent").value.trim();if(!content)return;form.classList.add("sending");messageBox.textContent=text("제보를 전송하고 있습니다.","Sending your report.");try{if(typeof window.SAEROM_SUBMIT_REPORT!=="function")throw new Error(text("서버 연결을 확인해 주세요.","Please check the server connection."));await window.SAEROM_SUBMIT_REPORT({type:$("#reportType").value,content,contact:$("#reportContact").value.trim(),page:location.href});form.reset();messageBox.textContent=text("오류 제보가 완료되었습니다. 알려주셔서 감사합니다.","Your report was submitted. Thank you.");toast(true,text("제보 완료","Report submitted"));}catch(error){messageBox.textContent=text(`제보를 보내지 못했습니다: ${error.message}`,`Could not submit: ${error.message}`);}finally{form.classList.remove("sending");}};}
